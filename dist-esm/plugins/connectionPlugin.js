...
import { eachObj, getOwnPackage, isObject, isPromiseLike, mapObj, pathToArray, printedGenTypingImport, } from '../utils';
...
const ForwardOnlyStrictArgs = Object.assign(Object.assign({}, ForwardPaginateArgs), { first: nonNull(intArg({ description: 'Returns the first n elements from the list.' })) });
...
const BackwardOnlyStrictArgs = Object.assign(Object.assign({}, BackwardPaginateArgs), { last: nonNull(intArg({ description: 'Returns the last n elements from the list.' })) });
...
export const connectionPlugin = (connectionPluginConfig) => {
    ...
    return plugin({
        ...
        onInstall(b) {
            ...
            if (pluginExtendEdge) {
                const edgeFields = mapObj(pluginExtendEdge, (val, key) => `${key}: connectionPluginCore.EdgeFieldResolver<TypeName, FieldName, "${key}">`);
                            ...
                            definition(t2) {
                                ...
                                if (pluginExtendConnection) {
                                    eachObj(pluginExtendConnection, (extensionFieldConfig, extensionFieldName) => {
                                        t2.field(extensionFieldName, Object.assign(Object.assign({}, extensionFieldConfig), { resolve: fieldConfig[extensionFieldName] }));
                                    });
                                }
                            ...
                            resolveFn = (root, args, ctx, info) => {
                                return completeValue(fieldConfig.resolve(root, args, ctx, info), (val) => {
                                    if (val && val.nodes === undefined) {
                                        return Object.assign(Object.assign({}, val), { nodes: completeValue(val.edges, (edges) => edges.map((edge) => edge.node)) });
                                    }
                                    return val;
                                });
                            };
...
export function makeResolveFn(pluginConfig, fieldConfig) {
    ...
    return (root, args, ctx, info) => {
        ...
        const resolvePageInfo = () => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c;
